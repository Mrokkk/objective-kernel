...
cpu::mmu::page_directory_entry kernel_page_dir[1024] ALIGN(PAGE_SIZE);
cpu::mmu::page_table_entry kernel_page_table[1024] ALIGN(PAGE_SIZE);
...
void initialize() {
    pmm.initialize(align(boot::upper_mem, 1024) * 1024, reinterpret_cast<uint32_t>(phys_address(sections::__heap_start)));
